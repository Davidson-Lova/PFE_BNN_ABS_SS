...
BNN consists of esmitimating the a posteriori distribition $p(\theta |D , \mathcal{M})$ of $\theta$ assuming the model $\mathcal{M}$ and the data set $D$ (whereas a standard FNN just gives one point estimation of $\theta$). Here we have an FNN on the left and BNN on the right.
